...
public final class Connection implements FrameSender {
    ...
    private boolean delegatedToHttp2 = false;
    ...
    private void processRequests() {
        while (!awaitingResponse) {
            int prefaceState = matchHttp2Preface();
            if (prefaceState == 0) {
                return;
            }
            if (prefaceState > 0) {
    ...
    private int matchHttp2Preface() {
        int check = Math.min(inLen, Http2Frame.CLIENT_PREFACE.length);
        for (int i = 0; i < check; i++) {
            if (inBuffer[i] != Http2Frame.CLIENT_PREFACE[i]) {
                return -1;
            }
        }
        if (inLen < Http2Frame.CLIENT_PREFACE.length) {
            return 0;
        }
        return 1;
    }

    private void delegateToHttp2() {
        if (delegatedToHttp2) {
            return;
        }
        delegatedToHttp2 = true;
        key.cancel();
        byte[] prefetched = Arrays.copyOf(inBuffer, inLen);
        inLen = 0;
        writeQueue.clear();
        awaitingResponse = false;
        closeAfterWrite = false;
        server.handOffToCleartextHttp2(channel, remoteIp, prefetched, true);
    }
